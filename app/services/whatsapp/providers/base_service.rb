...
class Whatsapp::Providers::BaseService
  ...
  def create_button_payload(message)
    ...
    create_payload('button', message.outgoing_content, action)
  end
  ...
  def create_list_payload(message)
    ...
    section1 = { 'rows' => rows }
    sections = [section1]
    action = { :button => I18n.t('conversations.messages.whatsapp.list_button_label'), 'sections' => sections }
    create_payload('list', message.outgoing_content, action)
  end
end
