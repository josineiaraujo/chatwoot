...
<template>
  <div class="relative box-border w-full min-w-0" :class="attrs.class">
    ...
    <button
      v-if="canClear"
      type="button"
      class="absolute top-1/2 z-10 grid size-6 -translate-y-1/2 place-content-center rounded-md bg-n-alpha-2 text-n-slate-12 transition-colors hover:bg-n-alpha-3 focus:outline-none focus:ring-1 focus:ring-n-brand ltr:right-9 rtl:left-9"
    ...
    <span
      aria-hidden="true"
      class="pointer-events-none absolute top-1/2 z-10 block size-4 -translate-y-1/2 text-n-slate-11 i-lucide-chevron-down ltr:right-3 rtl:left-3"
    />
  </div>
</template>
