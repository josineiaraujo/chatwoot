...
const buildStore = ({
  ...
  createStore({
    ...
    getters: {
      ...
      'accounts/isFeatureEnabledonAccount': () => () => macrosEnabled,
      'accounts/getAccount': () => () => ({}),
      'portals/allPortals': () => [],
...
describe('ReplyBox', () => {
  ...
  describe.each(CHANNELS)('$name', ({ name, inbox }) => {
    ...
    it.each(['open', 'resolved', 'snoozed'])(
      'blocks template sends until a bot-owned %s conversation is taken over',
      status => {
        const { wrapper } = mountWith({
          inbox,
          chat: {
        ...
        expect(bottomPanel(wrapper).enableWhatsAppTemplates).toBe(false);
        expect(bottomPanel(wrapper).isEditorDisabled).toBe(true);
      }
    );

    it('requires handoff before sending from a pending conversation', () => {
      const { wrapper } = mountWith({
        inbox,
        chat: {
          status: 'pending',
  ...
  it('enables macros after the conversation is assigned to the current agent', () => {
    const { wrapper } = mountWith({
      inbox: { channel_type: 'Channel::WebWidget' },
      macrosEnabled: true,
    });

    expect(editor(wrapper).enableMacros).toBe(true);
  });
